...
CFLAGS=-pg -g -ldl -I/include -Ofast -qopenmp -qopt-report=5 -Wall -std=gnu11 -Wextra -DNIX -D $(ARCHITECTURE) -D $(USE_OPT_LEVEL) -D $(USE_GENERATION_A) -D $(USING_OPENSSL)
ifeq "$(CC)" "gcc"
CFLAGS+= -march=native
endif
ifeq "$(USE_OPENSSL)" "FALSE"
...
LDFLAGS=-lm -L$(OPENSSL_LIB_DIR) -lssl -lcrypto -qopenmp
endif
...
KEM_FRODO640_HEADERS := api.h config.h frodo_macrify.h advisor-annotate.h

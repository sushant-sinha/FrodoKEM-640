...
#pragma omp parallel for num_threads(2)
	...
	int x=i*PARAMS_N;
        for (j = 0; j < PARAMS_N; j += PARAMS_STRIPE_STEP) {
            A[x + j] = i;                              // Loading values in the little-endian order
            A[x + j + 1] = j;                                  
        }
    }
	...
	shake128((unsigned char*)(A + i*PARAMS_N), (__vector unsigned long long)(2*PARAMS_N), seed_A_separated, 2 + BYTES_SEED_A);
    }
#endif    
